...
api_key = ""
if not api_key:
    st.error("❌ Please enter your Groq API key in the code.")
    st.stop()
...
def analyze_code_context(query: str) -> str:
    ...
    #For this demo, prepare the code for analysis
    return f"Anlayzing the following code snippet:\n{query}"
...
def generate_response_node(state: CodeState) -> CodeState:
    ...
    return {**state, "response": response_text, "messages": state["messages"] + [AIMessage(content=response_text)]}
